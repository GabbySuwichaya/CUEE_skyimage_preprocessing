...
folder_name = "h5files_Frame-%d-Mins_IMS-%s" % (int(frame_duration), Image_Size)
        ...
        if os.path.isfile("%s/%s.h5" % (folder_name, date_time_string)): 
            ...
            for image_path_name in input_image_names:
                ...
                if im_resize: 
                    ...
                    # plot_patch(axs[1], image_array_pil, "Predict-reshape : [%d]x[%d]" % (image_array_pil.shape[0],image_array_pil.shape[1]))
                    # plt.show()   
